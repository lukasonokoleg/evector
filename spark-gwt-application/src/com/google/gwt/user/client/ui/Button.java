...
public class Button extends ButtonBase {
     ...
     * +-----------------+
     * |+--------+       |
     * || Button | <-?-> |  <-  IE7 adds padding for button by text length, css "overflow:visible" hides padding,
     * |+--------+       |      however button's parent container is rendered by button width with padding.
     * +-----------------+      Solution is to set css width to button after it is attached
     * @author valdas.vaivaras
     */
    @Override
    protected void onAttach() {
        ...
        if (BrowserHelper.isIe7OrOlder()) {
            ...
            if (w != 0) {
                getElement().getStyle().setWidth(getElement().getOffsetWidth(), Unit.PX);
            }
        }
    }

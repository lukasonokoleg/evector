...
public class PublicSpringGwtRpcServlet extends RemoteServiceServlet {
    ...
    protected void addResponseHeadersAndCookies() {
        /*        UserSession session = springContext.getBean(UserSession.class);
                if (null != session) {
                    ...
                    Date accessTime = (session.getLastAccessTime() != null ? session.getLastAccessTime() : session.getCreationTime());
                    if (null != timeout && null != accessTime) {
                        long now = System.currentTimeMillis();
                        Long expiresAfter = accessTime.getTime() + timeout - now;
                        getThreadLocalResponse().setHeader("X-ALCS-SessionExpires", Long.toString(expiresAfter));
                    }
                }*/
        getThreadLocalResponse().setHeader("X-ALCS-SessionExpires", Long.toString(Long.MAX_VALUE));// TODO: session //
                                                                                                   // timeout
        setCookie(COOKIE_NAME_DEBUG_MODE, String.valueOf(gwtLog.isDebugEnabled()));
    }
